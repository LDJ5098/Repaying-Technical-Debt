...
위협 시나리오 기반 설계로 현대적인 보안 아키텍처를 구축한 1인 프로젝트입니다.
...
## 프로젝트 결과
- **[취약점 분석]** nmap+tshark+TLS 인증서 검증 우회 및 구독 공격을 구현(Python), 평문 통신의 데이터 탈취 취약점 확인
- **[피해 범위 최소화]** mTLS+PKI 기반 상호 인증 체계 구축 및 ACL 적용으로 만일 침입 성공 시에도 피해 범위 최소화
- **[구조 개선]** GitHub Actions 의존도 최소화를 위해 독립 승인체계 및 타임락, 배포 일시 정지, 롤백 기반의 배포 파이프라인 설계
- **[속도 향상]** 배포 속도 향상을 위해 트레이드 오프를 통한 병렬 구조 선택, 시스템 유휴 대기 시간 제거 (86.7% 향상)
- **[시뮬레이션]** Docker-DooD 구조로 과거 OS 환경 모방 시뮬레이션 및 Dev, infra 환경 격리